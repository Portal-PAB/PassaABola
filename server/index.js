...
// ROTA PARA BUSCAR DADOS COMPLETOS DE UM USUÁRIO
...
app.get('/api/usuario/minhas-inscricoes/:email', async (req, res) => {
  ...
  if (copaAberta) {
    ...
    // VERIFICA SE ESTÁ INSCRITA COMO AVULSA NA COPA
...
// Exportar inscritos em Excel
app.get('/api/encontros/:id/inscritos/excel', async (req, res) => {
  const encontroId = req.params.id;
  const dbInscricoesPath = path.join(dataDir, `encontro-${encontroId}-inscricoes.json`);
  const inscritos = await readJsonFile(dbInscricoesPath);
  ...
  inscritos.forEach(inscrito => {
    sheet.addRow(inscrito);
  });

  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", `attachment; filename=inscritos-encontro-${encontroId}.xlsx`);

  await workbook.xlsx.write(res);
  res.end();
});

// Exportar inscritos em PDF
app.get('/api/encontros/:id/inscritos/pdf', async (req, res) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename=inscritos-encontro-${encontroId}.pdf`);

  const doc = new PDFDocument();
  doc.pipe(res);

  doc.fontSize(18).text(`Inscritos do Encontro ${encontroId}`, { align: "center" });
  doc.moveDown();

  inscritos.forEach((inscrito, index) => {
    doc.fontSize(12).text(
      `${index + 1}. Nome: ${inscrito.nome} | CPF: ${inscrito.cpf} | Email: ${inscrito.email} | Telefone: ${inscrito.telefone || "N/A"}`
    );
    doc.moveDown(0.5);
  });
...
// Exportar inscritos da Copa em Excel
app.get('/api/copas/:id/inscritos/excel', async (req, res) => {
  ...
  sheetEquipes.columns = [
    { header: "Time", key: "nomeTime", width: 25 },
    { header: "Responsável", key: "responsavel", width: 25 },
    { header: "CPF", key: "cpf", width: 20 },
    { header: "Email", key: "email", width: 30 },
    { header: "Telefone", key: "telefone", width: 20 },
    { header: "Jogadoras", key: "jogadoras", width: 50 }
  ];

  equipes.forEach(equipe => {
    sheetEquipes.addRow({
      nomeTime: equipe.nomeTime,
      responsavel: equipe.responsavel,
      cpf: equipe.cpf,
      email: equipe.email,
      telefone: equipe.telefone || "N/A",
      jogadoras: equipe.jogadoras.map(j => `${j.nome} (${j.cpf})`).join(", ")
    });
  });

  sheetAvulsas.columns = [
    { header: "Nome", key: "nome", width: 30 },
    { header: "CPF", key: "cpf", width: 20 },
    { header: "Email", key: "email", width: 30 },
    { header: "Telefone", key: "telefone", width: 20 }
  ...
  jogadorasAvulsas.forEach(j => sheetAvulsas.addRow(j));
...
// Exportar inscritos da Copa em PDF
app.get('/api/copas/:id/inscritos/pdf', async (req, res) => {
  ...
  doc.fontSize(18).text(`Inscritos da Copa ${id}`, { align: "center" });
  doc.moveDown();


  doc.fontSize(14).text("Jogadoras Avulsas:");
  jogadorasAvulsas.forEach((j, i) => {
    doc.fontSize(12).text(`${i + 1}. ${j.nome} | CPF: ${j.cpf} | Email: ${j.email} | Tel: ${j.telefone || "N/A"}`);
  });
  doc.moveDown(2);


  doc.fontSize(14).text("Equipes:");
  equipes.forEach((equipe, i) => {
    doc.moveDown();
    doc.fontSize(13).text(`${i + 1}. ${equipe.nomeTime}`);
    doc.fontSize(11).text(
      `Responsável: ${equipe.responsavel} | CPF: ${equipe.cpf} | Email: ${equipe.email} | Tel: ${equipe.telefone || "N/A"}`
    );
    doc.text("Jogadoras:");
    equipe.jogadoras.forEach((j, idx) => {
      doc.text(`   - ${j.nome} (${j.cpf})`);
    });
  });
